...
const DoctorCard = ({ doctor }) => {
    ...
    return (
        ...
        >
            ...
            <div className="doctor-info p-4 bg-white dark:bg-gray-800 border border-t-0 border-gray-100 rounded-b-xl shadow-sm dark:shadow-none group-hover:shadow-md transition-shadow">
                <h3 className="doctor-name text-lg font-semibold text-gray-900 dark:text-white truncate">{doctor.name}</h3>
                <p className="doctor-specialty text-sm text-gray-600 dark:text-gray-300 mb-3">{doctor.speciality}</p>

                <div className="flex items-center gap-2 mt-2 flex-wrap text-xs text-gray-500 dark:text-gray-400">
                    <span className="badge-specialty flex items-center gap-1 bg-gray-50 dark:bg-gray-900 px-2 py-1 rounded">
                    ...
                    {doctor.experience && (
                        <span className="badge-experience flex items-center gap-1 bg-gray-50 dark:bg-gray-900 px-2 py-1 rounded">
                            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                ...
                {doctor.fees && (
                    <div className="fee-display mt-3 pt-3 border-t border-gray-50 flex items-center justify-between text-sm">
                        <span className="fee-label text-gray-500 dark:text-gray-400">Consultation Fee</span>
                        <span className="fee-amount font-semibold text-gray-900 dark:text-white">₹{doctor.fees}</span>
                    </div>
                )}
            </div>
        </div>
