...
const Navbar = () => {
  ...
  const { token, setToken, userData, userRole, setUserRole, theme, toggleTheme } = useContext(AppContext)
      ...
      <ul className='hidden md:flex items-center gap-6 font-medium'>
        {userRole !== 'doctor' && (
          <li>
            <NavLink
              to='/'
              className={({ isActive }) => `relative py-2 transition-all duration-300 ${isActive ? 'text-primary font-semibold' : 'text-gray-700 dark:text-gray-300 hover:text-primary'}`}
            >
        ...
        {userRole !== 'doctor' && (
          <li>
            ...
            >
              <span className="relative text-sm">
                Health Dashboard
                {location.pathname === '/health-dashboard' && <span className='absolute -bottom-1 left-0 w-full h-0.5 bg-gradient-primary rounded-full'></span>}
              </span>
            </NavLink>
          </li>
        )}
        ...
        {userRole === 'doctor' && (
          <>
            ...
            <li>
              <NavLink
                to='/teleconsultation?role=doctor'
                className={() => `relative py-2 transition-all duration-300 ${location.pathname === '/teleconsultation' ? 'text-primary font-semibold' : 'text-gray-700 dark:text-gray-300 hover:text-primary'}`}
              >
                <span className="relative text-sm flex items-center gap-1">
                  Live Chat
                  <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse shadow-soft"></span>
                  {location.pathname === '/teleconsultation' && <span className='absolute -bottom-1 left-0 w-full h-0.5 bg-gradient-primary rounded-full'></span>}
                </span>
              </NavLink>
            </li>
          </>
