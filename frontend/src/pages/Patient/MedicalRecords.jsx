        ...
        <div className='min-h-screen pt-8 pb-20 px-4 md:px-6'>
            <div className='max-w-7xl mx-auto'>
                <div className='relative overflow-hidden rounded-3xl border border-cyan-100 bg-gradient-to-r from-cyan-50 via-white to-emerald-50 p-6 md:p-8 mb-8'>
                    ...
                    <div className='relative flex flex-col lg:flex-row gap-6 lg:items-center lg:justify-between'>
                        <div>
                            ...
                            <h1 className='text-3xl md:text-4xl font-black text-slate-900 mb-2'>Medical Records</h1>
                            <p className='text-slate-600 max-w-xl'>Track consultations, diagnoses, and care history in one organized timeline.</p>
                        </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8'>
                    ...
                    <div className='rounded-2xl border border-slate-200 bg-white p-5 shadow-sm'>
                        <p className='text-sm text-slate-500 mb-1'>Latest Entry</p>
                        <p className='text-xl font-bold text-slate-900'>{latestRecordDate}</p>
                    </div>
                </div>
                    ...
                    <div className='flex flex-col md:flex-row gap-3'>
                        <div className='relative flex-1'>
                            ...
                            <input
                                ...
                                className='w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-400 outline-none transition-all'
                            />
                        </div>
                        <input
                            type='date'
                            ...
                            className='w-full md:w-auto px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-400 outline-none transition-all'
                        />
                        {hasFilters && (
                            <button
                                onClick={() => {
                                ...
                                className='px-4 py-3 rounded-xl border border-slate-200 bg-white text-slate-700 font-semibold hover:bg-slate-50 transition-colors'
                            >
                                Clear Filters
                            </button>
                        )}
                    ...
                    <div className='flex flex-wrap gap-2'>
                        {recordTypes.map((type) => (
                            ...
                            >
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded-full ${activeTab === type.value ? 'bg-white/20' : 'bg-slate-200 text-slate-600'}`}>{type.count}</span>
                            </button>
                        ))}
                    </div>
                </div>
                            ...
                            >
                                <div className='flex flex-col lg:flex-row lg:items-start gap-5'>
                                    ...
                                    <div className='flex-1'>
                                        ...
                                        <div className='grid grid-cols-1 md:grid-cols-3 gap-2 text-sm text-slate-600 mb-3'>
                                            <p><span className='font-semibold text-slate-700'>Date:</span> {new Date(record.date).toLocaleDateString()}</p>
                                            <p><span className='font-semibold text-slate-700'>Doctor:</span> {record.doctorName || 'Not specified'}</p>
                                            <p><span className='font-semibold text-slate-700'>Hospital:</span> {record.hospital || 'Not specified'}</p>
                                        </div>
                ...
                <div className='fixed inset-0 bg-black/55 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
                    <div className='bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-slate-200'>
                        <div className='px-6 py-5 border-b border-slate-200 bg-gradient-to-r from-cyan-50 to-emerald-50 flex items-center justify-between'>
                            <div>
                                <h2 className='text-2xl font-bold text-slate-900'>{isEditing ? 'Edit Medical Record' : 'Add Medical Record'}</h2>
                                <p className='text-sm text-slate-600 mt-1'>Keep your timeline updated and easy to review.</p>
                            </div>
                            <button
                        ...
                        <div className='p-6 md:p-8 overflow-y-auto w-full h-full'>
                            ...
                            <form onSubmit={handleAddRecord} className='space-y-4'>
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                    <div>
                                        <label className='block text-sm font-semibold text-slate-700 mb-1'>Record Title *</label>
                                        <input
                                            required
                                            type='text'
                                            name='title'
                                            ...
                                            className='w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-400 outline-none transition-all'
                                    ...
                                    <div className='md:col-span-2'>
                                        <label className='block text-sm font-semibold text-slate-700 mb-1'>Description/Symptoms *</label>
                                        <textarea
                                            required
                                            name='description'
                                    ...
                                    <div className='md:col-span-2'>
                                        {isEditing ? (
                                            <p className='text-sm text-slate-500 italic'>Editing attachments is currently not supported.</p>
                                        ) : (
                                            <>
                                                <label className='block text-sm font-semibold text-slate-700 mb-1'>Attachments (Photos/PDFs)</label>
                                                <input
                                                    type='file'
                                                    multiple
                                                    onChange={handleFileChange}
                                                    className='w-full p-3 bg-slate-50 border border-dotted border-cyan-300 rounded-xl transition-all'
                                                />
                                                <p className='text-[10px] text-slate-500 mt-1'>Max 5 files. Supported: Images, PDFs</p>
                                            </>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className='flex items-center justify-between mb-8 pb-4 border-b border-slate-100'>
                                <div className='flex items-center gap-4'>
                                    ...
                                    <div>
                                        ...
                                        <p className='text-slate-500'>{new Date(selectedRecord.date).toLocaleDateString()}</p>
                                    </div>
                                </div>
                                <button
                            ...
                            <div className='space-y-6'>
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                                    <div className='rounded-xl border border-slate-200 bg-slate-50 p-4'>
                                        ...
                                        <p className='font-semibold text-slate-800'>{selectedRecord.doctorName || 'Not specified'}</p>
                                    </div>
                                    <div className='rounded-xl border border-slate-200 bg-slate-50 p-4'>
                                        <p className='text-xs font-bold text-slate-400 uppercase tracking-wider mb-2'>Hospital</p>
                                        <p className='font-semibold text-slate-800'>{selectedRecord.hospital || 'Not specified'}</p>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className='p-4 bg-cyan-50 border border-cyan-100 rounded-xl'>
                                        <p className='text-slate-800 font-medium'>{selectedRecord.diagnosis || 'No diagnosis recorded'}</p>
                                    </div>
                                </div>

                                <div>
                                    <p className='text-xs font-bold text-slate-400 uppercase tracking-wider mb-2'>Description & Symptoms</p>
                                    <p className='text-slate-700 leading-relaxed'>{selectedRecord.description}</p>
                                </div>

                                {selectedRecord.notes && (
                                    <div>
                                        <p className='text-xs font-bold text-slate-400 uppercase tracking-wider mb-2'>Additional Notes</p>
                                        <p className='text-slate-600 p-4 bg-slate-50 rounded-xl border border-slate-100 italic'>{selectedRecord.notes}</p>
                                    </div>
                                )}
                                        ...
                                        <div className='flex flex-wrap gap-3'>
                                            {selectedRecord.attachments.map((file, idx) => (
                                                <a
                                                    ...
                                                    className='flex items-center gap-2 px-4 py-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-sm transition-all'
                                                >
                                                    <span>📄</span>
