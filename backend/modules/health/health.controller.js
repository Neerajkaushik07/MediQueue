...
import healthMetricModel from './healthMetric.model.js';
import medicalRecordModel from './medicalRecord.model.js';
import prescriptionModel from './prescription.model.js';
import labReportModel from '../advancedHealth/labReport.model.js';
...
const deleteHealthMetric = async (req, res) => {
    try {
        ...
        const deletedMetric = await healthMetricModel.findOneAndDelete({ _id: metricId, userId: req.body.userId });
        
        if (!deletedMetric) {
            return res.json({ success: false, message: "Metric not found or unauthorized" });
        }
