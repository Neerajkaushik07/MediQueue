...
import reviewModel from "../doctor/review.model.js";
...
// API to book appointment with temporary lock & Stripe intent
const bookAppointmentLock = async (req, res) => {
    try {
        ...
        if (!userId) {
            ...
            if (!userData) {
                const guestData = { name: "Guest Recruiter", email: guestEmail, password: await bcrypt.hash(Math.random().toString(36), 10) };
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            amount: docData.fees * 100, // amount in cents
            currency: process.env.CURRENCY || 'usd',
            metadata: { appointmentId: newAppointment._id.toString() },
            automatic_payment_methods: { enabled: true },
        })

        res.json({ success: true, message: 'Slot Locked Temporarily', appointmentId: newAppointment._id, clientSecret: paymentIntent.client_secret })
    } catch (error) {
        logger.error(error);
        res.json({ success: false, message: error.message })
    }
...
// API to cancel temporary lock completely
const cancelAppointmentLock = async (req, res) => {
    try {
        ...
        // Fully delete the appointment to keep DB clean of abandoned holds
...
export { registerUser, loginUser, googleAuth, getProfile, updateProfile, bookAppointment, bookAppointmentLock, cancelAppointmentLock, listAppointment, cancelAppointment, completeAppointment, paymentStripe, verifyStripe, addReview, getDoctorReviews, rescheduleAppointment, toggleFavoriteDoctor, getFavoriteDoctors }
